...
First part of this project, will be to test if there is a correlation between patients with PCOS using the body measurments data.
...
body_measurements %>% 
  ...
  labs(x = "Age groups", 
       y = "Hair growth (yes/no))",
       title = "Correlation between age groups and hair growth stratisfied for PCOS diagnosis",
       color = "PCOS diagnosis") +
  scale_color_brewer(palette = "Set2") +
  theme_light() + 
  theme(title = element_text(size = 8),
...
p_age_hair <- body_measurements |> 
  ggplot(mapping = aes(x = hair_growth)) +
  geom_bar() +
...
# FIX JULIE AND MILLE + tjek mille noter for et sejt plot (PRG values in a box plot stratisfied for PCOS and facet wrap it with something)
...
p_age_pimple <- body_measurements |> 
  ...
  labs(x = "Age groups", 
       y = "Pimples (yes/no))",
       title = "Correlation between age groups and experienced pimples",
       color = "PCOS diagnosis") +
  scale_color_brewer(palette = "Set2") +
  theme_light() +
  theme(title = element_text(size = 8),

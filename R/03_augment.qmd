...
body_measurements <- body_measurements |>
  mutate(blood_type = case_when(
    ...
    blood_type == 17 ~ "AB+",
    blood_type == 18 ~ "AB-")) |> 
  mutate(blood_type=factor(blood_type, levels= c("A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-")))
...
blood_measurements_updated <- blood_measurements %>%
  mutate(cycle_or_pregnancy_stage = case_when(
    ...
    TRUE ~ "Unknown"                                          # Default case for out-of-range values
  )) |> 
  mutate(cycle_or_pregnancy_stage = factor(cycle_or_pregnancy_stage, levels= c("Follicular Stage", "Likely Follicular Stage", "Luteal Stage", "Luteal Stage/Possibly First or Second Trimester Pregnancy", "First or Second Trimester Pregnancy", "Second Trimester Pregnancy", "Second or Third Trimester Pregnancy", "Third Trimester Pregnancy", "Unknown")))

# View the first few rows to check the new column
head(blood_measurements_updated)
```
